...
use crate::{
    features::user_auth::{
        application::repository::session::SessionRepositoryImpl,
        domain::session::{Session, entity::SessionEntity},
    },
    shared::infrastructure::persistence::RedisRepositoryImpl,
};
    ...
    App: Has<Namespace>,
{
    ...
    NAMESPACE.get_or_init(|| {
        app.get_dependency().nest("session")
    })
}
...
impl SessionRepositoryImpl for RedisRepositoryImpl {
    ...
    {
        let mut connection =
            app.get_dependency().get().await?;

        let (entity_type, entity_id) =
            source.entity.as_tuple();
        ...
        connection
            .set_ex(
                ...
                source.id.to_string(),
                Session::LIFETIME.try_into().context(
                    "while converting session lifetime",
                )?,
            )
            .await?;
